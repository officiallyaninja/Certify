...
def init(stdscr):
    ...
    curses.init_pair(3, curses.COLOR_YELLOW,
                     curses.COLOR_BLACK)  # selectable fields
    main_screen(stdscr)
...
def main_screen(win):  # View Events
    ...
    while True:
        win.clear()
        x, y = 0, 0
        win.addstr(y, x, "======= Certify CLI v1.0 =======",
                   curses.color_pair(1))
        y += 1
        win.addstr(
            y, x, "Navigate [up/down arrows] | Register New Event [+] | Quit [q]", curses.color_pair(1))
        y += 2
        ...
        if events_list == []:
            ...
            for idx, item in enumerate(events_list):
                if idx == selected_row_idx:
                    win.addstr(
                        y, x, f"{item['_id']} {item['name']}", curses.color_pair(2))
                else:
                    if item["issueDt"] == None:  # check finalized
                        win.addstr(
                            y, x, f"{item['_id']} {item['name']}", curses.color_pair(3))
                    else:
...
def reg_event(win):
    ...
    win.addstr(y, x, "Fields - seperated by commas(,) : ",
               curses.color_pair(1))
    ...
    response = requests.post(
        'http://localhost:6969/admin/add/event', params=data)
    # print(response.text)
    ...
    while True:
        ...
        if key == 81 or key == 113:  # Quit
            ...
            if not finalized:
                y += 1
                win.addstr(
                    y, x, "Are you sure? [y/n] : ", curses.color_pair(2))
                curses.echo()
                val = win.getstr().decode("utf-8")
                curses.noecho()
                if val.lower() == "y":
            ...
            if not finalized:
                if menu_items[selected_index][1]:  # if editable
                    ...
                    if selected_index == 4:
                        ...
                        y += 2
                        win.addstr(
                            y, x, "## 'name' and 'event_id' are present as default fields and cannot be deleted ##", curses.color_pair(3))
                        y += 1
                        win.addstr(
                            y, x, "Enter required fields seperated by commas(,) : ", curses.color_pair(2))
                        curses.curs_set(True)
                        curses.echo()
                        val = win.getstr().decode("utf-8")
                        curses.noecho()
                        ...
                        if val == "":
                            ...
                            fields_list = [item.strip()
                                           for item in val.split(',')]
                        db.events.update_one({"_id": event_id}, {
                                             "$set": {menu_items[selected_index][2]: fields_list}})
                    else:
                        y += 2
                        win.addstr(y, x, "Enter New Value : ",
                                   curses.color_pair(2))
                        curses.curs_set(True)
                        curses.echo()
                        val = win.getstr().decode("utf-8")
                        ...
                        curses.curs_set(False)
                        db.events.update_one({"_id": event_id}, {
                                             "$set": {menu_items[selected_index][2]: val}})

                    # possibility of this being executed before update
                    db_result = db.events.find_one({"_id": event_id})
                    menu_items = [[f"ID : {db_result['_id']}", False],
                                  [f"Event Name : {db_result['name']}",
                                      True, 'name'],
                                  [f"Description : {db_result['desc']}",
                                      True, 'desc'],
                                  [f"Issue Date : {db_result['issueDt']}", False],
                                  [f"Participant Fields : {db_result['fields']}", True, 'fields']]
        elif key in [100, 68]:
...
def viewParticipants(win, event_id, finalized):
    ...
    participants_list = list(db.participants.find(
        {"event_id": event_id}, {"_id": 1, "name": 1}))
    selected_row_idx = 0  # initially select index
    ...
    while True:
        ...
        if not finalized:
            win.addstr(
                y, x, "Add Participant [+] | Add Via CSV [~] | Delete All Participants [D] | Go Back [Q]", curses.color_pair(1))
            y += 2
        else:
        ...
        if key == 81 or key == 113:  # Quit
            ...
            viewParticipant(
                win, participants_list[selected_row_idx]["_id"], finalized)
            participants_list = list(db.participants.find(
                {"event_id": event_id}, {"_id": 1, "name": 1}))
...
def addParticipantCSV(win, event_id):
    ...
    # possible error where header names dont match up , use value inside fields to cross check
    ...
    win.addstr(y, x, "Added successfully | Press any key to continue ",
               curses.color_pair(1))
    win.getch()
...
def viewParticipant(win, participant_id, finalized):
    ...
    for field, value in db_result.items():
        if field in ["_id", "event_id"]:
            ...
            menu_items.append([f"{field} : {value}", False])
        else:
    ...
    while True:
        ...
        if not finalized:
            win.addstr(
                y, x, f"[MODIFIABLE] | Delete Participant [D] | Go Back [Q]", curses.color_pair(1))
            y += 2
            ...
            win.addstr(y, x, f"[FINALIZED] | Go Back [Q]",
                       curses.color_pair(1))
            y += 2
        ...
        if key == 81 or key == 113:  # Quit
            ...
            if not finalized:
                if menu_items[selected_index][1]:  # if editable
                    ...
                    curses.noecho()
                    curses.curs_set(False)
                    db.participants.update_one({"_id": participant_id}, {
                                               "$set": {menu_items[selected_index][2]: val}})
                    db_result = db.participants.find_one(
                        {"_id": participant_id})
                    menu_items = []
                    for field, value in db_result.items():
                        if field in ["_id", "event_id"]:
